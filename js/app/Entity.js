define(['app/Vector', 'app/Circle', 'app/Settings'], function(Vector, Circle, Settings) {
    return function(sprite, radialCenterPosition, rotationBounds) {
        ...
        that.appliedForce = new Vector(); //todo protect
        that.selected = false;
        ...
        that.getBoundingCircle = function(c) {
            if(bounds === null) return new Circle(sprite.position, 16); //todo magic number
            return new Circle(new Vector(sprite.position.x + bounds.position.x, sprite.position.y + bounds.position.y), bounds.radius);
        };
        ...
        that.update = function(elapsedTimeSeconds) {
            if(typeof(radialCenterPosition) !== "undefined") {
                ...
                var r = new Vector(currentBounds.position.x - radialCenterPosition.x, currentBounds.position.y - radialCenterPosition.y);
                ...
                sprite.position = new Vector(r.x + radialCenterPosition.x - bounds.position.x, r.y + radialCenterPosition.y - bounds.position.y);
                if(typeof(rotationBounds) !== "undefined" && rotationBounds > 0) {
                    //todo cleanup
                ...
                if(Math.abs(dist1) > Math.abs(dist2)) {
                    that.appliedForce = new Vector(-r.y,r.x);
                ...
                var acceleration = new Vector(that.mass * that.appliedForce.x, that.mass * that.appliedForce.y);
